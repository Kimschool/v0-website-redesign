<?php
// PHP 5.6: contact form → mail() multipart (text/plain + text/html)
// 通知メールのラベル・表記は日本語固定（確認者向け）
...
if ($fromName !== '') {
  $fromHeader = mail_utf8_display_name($fromName) . ' <' . $fromEmail . '>';
} else {
...
// 申請者への自動返信（ルート send.php PHPMailer 経路と同文面。有効なメールがあるときのみ）
if ($userEmail !== '') {
  $autoReplySubjectRaw = env_or_default('MAIL_AUTOREPLY_SUBJECT', '【KCP】申請ありがとうございます');
  $autoReplySubjectEncoded = mail_utf8_subject($autoReplySubjectRaw);
  ...
  $autoOk = false;
  if ($envelopeFrom !== '' && filter_var($envelopeFrom, FILTER_VALIDATE_EMAIL) !== false) {
    $autoOk = @mail($userEmail, $autoReplySubjectEncoded, $autoMimeBody, $autoHeadersStr, '-f ' . escapeshellarg($envelopeFrom));
  } else {
    $autoOk = @mail($userEmail, $autoReplySubjectEncoded, $autoMimeBody, $autoHeadersStr);
  }
  if (!$autoOk) {
    json_out(500, array('ok' => false, 'error' => 'Mail send failed'));
  }
